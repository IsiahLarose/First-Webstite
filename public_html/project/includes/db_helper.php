...
class DBH{
    ...
    public static function save_survey($survey){
        try {
             ...
             */
            $query = file_get_contents(__DIR__ . "/../sql/queries/CreateSurvey.sql");
            $stmt = DBH::getDB()->prepare($query);
            $stmt->execute([
                ":name"=>Common::get($survey, "name", null),
                ":desc"=>Common::get($survey, "description", null),
                ":apd"=>Common::get($survey, "attempts_per_day", 1),
                ":ma"=>Common::get($survey, "max_attempts", 1),
                ":um"=>Common::get($survey, "use_max", false)?1:0,//convert to tinyint
                ":uid"=>Common::get_user_id()
            ]);
            DBH::verify_sql($stmt);
            //get id
            $survey_id = DBH::getDB()->lastInsertId();
            ...
            for($i = 0; $i < $qt; $i++){
                ...
                if(($i+1) < $qt) {
                    $ni = $i + 1;
                    $query .= ", (:question$ni, :user_id, :survey_id)";
                }
            }
    ...
    public static function get_questionnaire_by_id($survey_id){
        try {
            //need to use a workaround for PDO
            $query = file_get_contents(__DIR__ . "/../sql/queries/get_full_questionnaire.sql");
            $stmt = DBH::getDB()->prepare($query);
            $result = $stmt->execute([":survey_id"=>$survey_id]);//not using associative array here
            DBH::verify_sql($stmt);
            if ($result) {
    ...
    public static function check_survey_status($survey_id){
        try {
            $query = file_get_contents(__DIR__ . "/../sql/queries/check_survey.sql");

            $user_id = Common::get_user_id();
            $stmt = DBH::getDB()->prepare($query);
            $result = $stmt->execute([":sid"=>$survey_id, ":uid"=>$user_id]);
            DBH::verify_sql($stmt);
            if($result){
    ...
    public static function save_response($survey_id, $response){
        try {
